...
import { getMarketContract } from "../../services/contract";
...
const CreateItem = () => {
  ...
  const [assetFileType, setAssetFileType] = useState('video/mp4');
  const [preview_image, setPreviewImage] = useState(null);
  const [pendingText, setPendingText] = useState('');
  const [id, setId] = useState(-1);
  const [check_create, setCheckCreate] = useState(true);

  const handleSave = async () => {
    if (!check_create) {
      toast.error("You can't create new item of selected collection");
      return;
    }
    if (account === null) {
    ...
    try {
      ...
      const tx = await nftContract.methods.createItem(details_hash.path).send({ from: account })
      ...
      formData.append('asset_filetype', assetFileType);
      formData.append('preview_image', preview_image);
      formData.append('tokenId', newTokenId);
      formData.append('ipfs_cid', "https://ipfs.io/ipfs/" + asset_hash.path);

      dispatch(saveItem(formData, account, router))
      // } else {
  ...
  const captureFile = (event, asset_type) => {
    ...
    if (file) {
      ...
      if (extNames.length > 1) {
        ...
        if (asset_type === 'video') {
          if (video_extList.indexOf(ext) > -1 || image_extList.indexOf(ext) > -1) {
            setAssetFileType(file.type)
  ...
  const handleChangeCollection = async (e) => {
    try {
      await axios.post(process.env.REACT_APP_ENDPOINT + "/item/check_create", { collection: e.target.value });
          ...
          <div className="row">
            <div className="col-lg-8 offset-lg-2 col-12">
              <div className="field-set">
                ...
                <select className="form-control" onChange={handleChangeCollection}>
                ...
                <h5>Upload asset <span className="color-red">*</span></h5>
                {
                  video_selected ? (
                    <div className="d-create-file position-relative">
                      {
                        assetFileType.indexOf('image') > -1 ? (
                          <img src={URL.createObjectURL(item_asset)} alt="" width="100%" height="100%" className='profile-preview' />
                        ) : (
                    ...
                    <div className="d-create-file">
                      <div>
                        <p id="file_name">Image or Video file allowed. Max 200mb.</p>
